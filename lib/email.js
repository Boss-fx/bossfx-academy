...
function courseEmailContent(customer, product, txRef, downloadToken) {
    let downloadSection = '';
    if (downloadToken) {
        const downloadUrl = `https://www.bossfxcademy.com/api/download?token=${downloadToken}`;
        downloadSection = `
    <div style="background:linear-gradient(135deg,#0f172a,#1a2332);border:2px solid #f59e0b;border-radius:12px;padding:24px;margin:24px 0;text-align:center;">
      <p style="margin:0 0 4px;color:#f59e0b;font-weight:700;font-size:16px;">YOUR COURSE FILES</p>
      <p style="margin:0 0 16px;color:#94a3b8;font-size:13px;">Download your materials instantly. Link valid for 72 hours.</p>
      ...
      <p style="margin:12px 0 0;color:#64748b;font-size:11px;">Need a new link? Contact hello@bossfxcademy.com</p>
    </div>`;
    } else {
        try {
            const { generateToken } = require('../api/download-forex101');
...
function vipEmailContent(customer, product, txRef, downloadToken) {
    let portalSection = '';
    if (downloadToken) {
        const portalUrl = `https://www.bossfxcademy.com/vip/welcome.html?token=${downloadToken}`;
        const downloadUrl = `https://www.bossfxcademy.com/api/download?token=${downloadToken}`;
        portalSection = `
    <div style="background:linear-gradient(135deg,#10b98122,#8b5cf622);border:2px solid #10b981;border-radius:12px;padding:24px;margin:24px 0;text-align:center;">
      <p style="margin:0 0 4px;color:#10b981;font-weight:700;font-size:16px;">YOUR VIP PORTAL</p>
      <p style="margin:0 0 16px;color:#94a3b8;font-size:13px;">Access all your downloads, book mentorship, and more.</p>
      <a href="${portalUrl}" style="display:inline-block;background:#10b981;color:#fff;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:700;font-size:16px;">
        Enter VIP Portal
      </a>
      <p style="margin:12px 0 0;color:#64748b;font-size:11px;">Direct download: <a href="${downloadUrl}" style="color:#f59e0b;">Download All Files</a></p>
    </div>`;
    }
...
function eaEmailContent(customer, product, txRef, downloadToken) {
    let downloadSection = '';
    if (downloadToken) {
        const downloadUrl = `https://www.bossfxcademy.com/api/download?token=${downloadToken}`;
        downloadSection = `
    <div style="background:linear-gradient(135deg,#0f172a,#1a2332);border:2px solid #10b981;border-radius:12px;padding:24px;margin:24px 0;text-align:center;">
      <p style="margin:0 0 4px;color:#10b981;font-weight:700;font-size:16px;">DOWNLOAD YOUR EA FILES</p>
      <p style="margin:0 0 16px;color:#94a3b8;font-size:13px;">Includes EA file + settings guide. Link valid for 72 hours.</p>
      <a href="${downloadUrl}" style="display:inline-block;background:#10b981;color:#fff;padding:14px 36px;border-radius:8px;text-decoration:none;font-weight:700;font-size:16px;">
        Download EA Bundle
      </a>
    </div>`;
...
// ----------------------------------------------------------------
// EA Addon Section (appended to non-EA emails when EA is purchased)
// ----------------------------------------------------------------

function eaAddonSection(eaDownloadToken) {
    let downloadBtn = '';
    if (eaDownloadToken) {
        const eaDownloadUrl = `https://www.bossfxcademy.com/api/download?token=${eaDownloadToken}`;
        downloadBtn = `
      <a href="${eaDownloadUrl}" style="display:inline-block;background:#10b981;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;margin-top:12px;">
        Download EA Files
      </a>
      <p style="margin:8px 0 0;color:#64748b;font-size:11px;">EA download link valid for 72 hours</p>`;
    }

    return `
    <div style="background:linear-gradient(135deg,#f59e0b11,#d9770611);border:2px solid #f59e0b;border-radius:12px;padding:24px;margin:28px 0 8px;text-align:center;">
      <p style="margin:0 0 4px;color:#f59e0b;font-weight:700;font-size:15px;text-transform:uppercase;letter-spacing:0.5px;">BONUS: SMA Pro Trend EA Included</p>
      <p style="margin:0 0 8px;color:#94a3b8;font-size:13px;">Your order includes the SMA Pro Trend EA (₦15,000 value). Download it below or from MQL5 Market.</p>
      ${downloadBtn}
      <div style="margin-top:14px;">
        <a href="https://www.mql5.com/en/market/product/174970" style="color:#f59e0b;font-size:13px;text-decoration:underline;">Get from MQL5 Market</a>
      </div>
    </div>

    <div style="background:#0f172a;border-radius:8px;padding:16px 20px;margin:12px 0 0;">
      <p style="margin:0 0 10px;color:#f59e0b;font-weight:600;font-size:13px;">EA QUICK START</p>
      <ol style="color:#cbd5e1;font-size:13px;line-height:1.8;padding-left:18px;margin:0;">
        <li>Open MetaTrader 5 → File → Open Data Folder</li>
        <li>Copy the EA file into MQL5/Experts</li>
        <li>Restart MT5 and attach the EA to your chart</li>
        <li>Enable AutoTrading and use recommended settings</li>
      </ol>
    </div>`;
}
...
function adminNotificationContent(customer, product, txRef, paymentData) {
    ...
    <div style="background:#0f172a;border-radius:8px;padding:20px;margin:24px 0;">
      <table width="100%" style="color:#cbd5e1;font-size:14px;">
        ...
        <tr><td style="padding:6px 0;font-weight:600;color:#f59e0b;">EA Addon</td><td style="text-align:right;color:${(paymentData.meta?.ea_bundle === 'yes' || paymentData.meta?.has_ea_addon) ? '#22c55e' : '#64748b'};font-weight:600;">${(paymentData.meta?.ea_bundle === 'yes' || paymentData.meta?.has_ea_addon) ? 'YES (+₦15,000)' : 'No'}</td></tr>
 ...
 * @param {object} eaOptions - { hasEaAddon: bool, eaDownloadToken: string|null }
 */
async function sendFulfillmentEmail(customer, product, txRef, downloadToken, eaOptions) {
    const client = getBrevoClient();
    const { hasEaAddon, eaDownloadToken } = eaOptions || {};
    ...
    let emailBody = contentFn(customer, product, txRef, downloadToken);

    // Append EA addon section for non-EA products that include the addon
    if (hasEaAddon && product.type !== 'ea') {
        emailBody += eaAddonSection(eaDownloadToken);
    }

    const htmlContent = baseTemplate(emailBody, product);
