...
const SEQUENCES = {
    ...
    challenge: {
        ...
        listKey: 'challenge',
        steps: [
            { template: 'challenge_1', delay: 0 },     // Day 0 — welcome + how it works
            { template: 'challenge_2', delay: 72 },    // Day 3 — the early slip
            { template: 'challenge_3', delay: 168 },   // Day 7 — week 1 milestone
            { template: 'challenge_4', delay: 336 },   // Day 14 — Forex 101 bridge
            { template: 'challenge_5', delay: 504 },   // Day 21 — push through the wall
            { template: 'challenge_6', delay: 720 }    // Day 30 — completion + mentorship
        ]
    },
    mentorship: {
    ...
    // ---- LMS student journeys (P1–P5) ----
    student_welcome: {           // P1 — on free /learn/ signup
        name: 'Student Welcome',
        listKey: 'general',
        steps: [
            { template: 'student_welcome_1', delay: 0 },   // Immediate
            { template: 'student_welcome_2', delay: 48 }    // Day 2
        ]
    },
    free_to_paid: {              // P2 — finished free modules, not enrolled
        name: 'Free to Paid Conversion',
        listKey: 'general',
        steps: [
            { template: 'free_to_paid_1', delay: 0 },
            { template: 'free_to_paid_2', delay: 48 },
            { template: 'free_to_paid_3', delay: 120 }
        ]
    },
    onboarding: {                // P4 — post-purchase → mentorship upsell
        name: 'Forex 101 Onboarding',
        listKey: 'general',
        steps: [
            { template: 'onboarding_1', delay: 0 },
            { template: 'onboarding_2', delay: 72 },
            { template: 'onboarding_3', delay: 240 }
        ]
    },
    student_reengage: {          // P3 — inactive student (cron-triggered)
        name: 'Student Re-engagement',
        listKey: 'general',
        steps: [
            { template: 'student_reengage_1', delay: 0 },
            { template: 'student_reengage_2', delay: 72 }
        ]
    },
    checkout_recovery: {         // P5 — abandoned checkout (cron-triggered)
        name: 'Checkout Recovery',
        listKey: 'general',
        steps: [
            { template: 'checkout_recovery_1', delay: 0 },
            { template: 'checkout_recovery_2', delay: 24 }
        ]
    }
};
...
function getSequenceForSource(source) {
    ...
    // LMS student journeys (checked first — specific intents)
    if (s.includes('student_signup') || s.includes('lms_signup')) return 'student_welcome';
    if (s.includes('free_completed') || s.includes('free_done')) return 'free_to_paid';
    if (s.includes('enrolled') || s.includes('forex101_purchase') || s.includes('purchase_forex')) return 'onboarding';
    if (s.includes('checkout')) return 'checkout_recovery';
    if (s.includes('student_reengage')) return 'student_reengage';
