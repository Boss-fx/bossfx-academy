...
const { grantEnrollmentsForProduct } = require('./enrollment');
...
async function fulfillOrder(paymentData) {
    ...
    // ---- 4b. Grant course access (unlocks paid modules in /learn/) ----
    try {
        const grant = await grantEnrollmentsForProduct(customer.email, product.id, txRef, 'flutterwave');
        if (grant.granted && grant.granted.length) {
            console.log(`[Fulfillment] Course access granted: ${grant.granted.join(', ')} for ${customer.email}`);
        }
    } catch (err) {
        console.error(`[Fulfillment] Enrollment grant failed (non-fatal):`, err.message);
    }
